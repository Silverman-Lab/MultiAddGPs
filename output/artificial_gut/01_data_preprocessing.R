...
source('~/src/00_package_loading.R', chdir = TRUE)
source('~/src/helper_functions.R', chdir = TRUE)
load("~/data/mallard_family.RData")
set.seed(59292)

mallard_family$sample_data <- mallard_family$sample_data %>% 
    dplyr::select(X.SampleID,time,Vessel,SampleType,batch)%>%
    group_by(Vessel)%>%
    arrange(time)%>%
...
X<- t(model.matrix(~ Vessel1+Vessel2+Vessel3+Vessel4+Vessel1Spike+Vessel2Spike+Veseel1Burn+Vessel2Burn+Hour_diff+0,data = X1)) # Transpose to get the right dimensions
