...
source('~/artificial_gut/02_analysis.R', chdir = TRUE)
...
p <- ggplot(predicted_clr_tidy,aes(x=Hour,y=mean))+
      ...
      theme(
        ...
        axis.title.y = element_text(size = 35, margin = margin(t = 15)),
      ...
      labs(y = "F", x= "Hour")+
      annotate("rect", xmin = 240, xmax = 264, ymin = -Inf, ymax = Inf, fill = "#717D7E", alpha = 0.25) 


ggsave("~/AllVessels.png", plot = p,width = 35,height = 18)
...
ggsave("~/Disruption.png", plot = disruption_plot,width = 35,height = 18)
